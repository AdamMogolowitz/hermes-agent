...
为何需要此 hook：内置平台（Telegram、Discord、Slack 等）在 `tools/send_message_tool.py` 中内置了直接 REST 辅助函数，使 cron 无需在同一进程中持有 gateway 即可投递。Plugin 平台历史上依赖 `gateway.runner_registry` 的 `get_gateway_runner()`（或 `gateway.run` 中的旧版 `_gateway_runner_ref()` 垫片），在 gateway 进程外会返回 `None`，因此若没有 `standalone_sender_fn`，cron 端发送会失败并报 `No live adapter for platform '<name>'`。
