...
`delegated_start_notifications` is gateway-only. The key appears in global `config.yaml` defaults for discoverability, but the CLI and TUI ignore it — only the messaging gateway reads and applies it. When enabled (default **on** for all messaging platforms), Hermes sends a one-shot bubble when `delegate_task` starts a subagent, showing the **configured** model name at spawn time and the goal. This is independent from `tool_progress` — you can keep tool breadcrumbs off (`tool_progress: off`) while still seeing which model a delegation picked, or disable bubbles globally with `delegated_start_notifications: false`. Per-platform opt-out: `display.platforms.<platform>.delegated_start_notifications: false`. Programmatic surfaces (`local`, `api_server`, `webhook`, `msgraph_webhook`) never receive these bubbles. The configured model label reflects the child agent's model at spawn, not runtime fallback after the first API call.

Delegated-start bubbles require an **in-process** gateway agent run (the `_run_agent_inner` progress-callback path). They are **not available** when `gateway.proxy_url` / `GATEWAY_PROXY_URL` is set (thin relay / proxy mode) — the same class of limitation as other gateway-only progress features in proxy setups.
